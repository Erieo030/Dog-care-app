...
INTENT_KEYWORDS = {
    ...
    "medication_active": ("目前吃什麼藥", "現在吃什麼藥", "目前用藥", "現在用藥"), "medication_history": ("用藥歷史", "以前吃過什麼藥"),
    "medical_latest": ("上次看醫生", "最近就醫", "去哪間醫院"), "medical_recent": ("就醫原因", "看診原因"),
    "reminder_today": ("今天提醒", "今日提醒"), "reminder_upcoming": ("下一個提醒", "接下來提醒"), "reminder_overdue": ("逾期提醒", "過期提醒"),
    ...
    "health_event_recent": ("健康異常", "最近異常", "嘔吐"), "health_monitor": ("健康觀察", "提醒我哪些", "為什麼出現"), "health_summary": ("整體狀況", "健康摘要", "最近狀況"),
    "pet_profile": ("品種", "幾歲", "性別", "結紮", "毛孩資料"),
}
UNSAFE = ("確診", "是不是", "是不是得了", "什麼病", "應該吃什麼藥", "推薦藥", "藥多少", "藥吃多少", "劑量", "停藥", "加藥", "處方")
...
class ChatService:
    ...
    def _facts(self, intent: str, context: AIContext, monitor: HealthMonitorResult) -> tuple[str, list[ChatSource], dict[str, Any]]:
        ...
        elif intent == "health_summary": facts=HealthSummaryService._prompt_context(context, monitor); sources=[_source("health_monitor","健康摘要")]
        return intent, sources, facts

    def _format(self, intent: str, facts: dict[str, Any], context: AIContext) -> str:
        if intent == "unknown": return "目前我可以協助查詢毛孩的體重、日常紀錄、健康異常、就醫、用藥、疫苗、驅蟲與提醒資訊。"
        if intent == "medical_advice_request": return "PawLog 無法根據紀錄確診疾病或提供用藥建議。你可以改問最近的健康紀錄；若症狀持續、惡化或感到擔心，請聯絡獸醫。"
        ...
        if intent == "health_summary": return f"{context.period.days} 天內已整理體重、日常紀錄、健康異常與照護資料；請參考健康摘要卡片。"
    ...
    async def answer(self, message: str, context: AIContext, monitor: HealthMonitorResult) -> dict[str, Any]:
        ...
        if self.provider and intent in {"health_summary", "unknown"} and getattr(self.provider, "available", False):
            try:
