...
def save(pid,uid,data):
 ...
 values={**data.model_dump(),"petId":pid,"publicToken":token,"updatedAt":now};values.setdefault("createdAt",now)
...
def public_html(token):
 ...
 fields=''.join(f'<p><b>{escape(k)}：</b>{escape(str(v))}</p>' for k,v in x.items() if v not in (None,"",False) and k not in {"name","lostMode"})
 return f"<!doctype html><html lang=zh-Hant><meta name=viewport content=width=device-width,initial-scale=1><title>PawLog 毛孩協尋</title><body><main><h1>🐾 PawLog 毛孩協尋</h1><h2>{name}</h2>{status}{fields}</main></body><style>body{{font-family:system-ui;padding:24px;background:#fffaf5}}main{{max-width:520px;margin:auto;background:white;padding:24px;border-radius:18px;box-shadow:0 2px 14px #ddd}}h1{{color:#6b4f3b}}h2{{color:#b6533c}}</style></html>"