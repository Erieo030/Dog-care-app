...
export default function WeightTrendChart({ items }: { items: DashboardWeightPoint[] }) {
  ...
  const width = 310,
    ...
    top = 12,
    bottom = 28;
  const values = points.map((x) => x.weightKg),
  ...
  const coords = points.map((item, index) => ({
    ...
    y: top + ((max - item.weightKg) / (max - min || 1)) * (height - top - bottom),
  }));
  return (
    <View style={s.wrap}>
      <Svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        />
        <SvgText x={2} y={top + 5} fontSize="10" fill={Colors.subtext}>
          {max.toFixed(1)}
        </SvgText>
        <SvgText x={2} y={height - bottom + 4} fontSize="10" fill={Colors.subtext}>
          {min.toFixed(1)}
        </SvgText>
        <Polyline
        ...
        />
        {coords.map((p) => (
          <Circle key={p.item.id} cx={p.x} cy={p.y} r="4" fill={Colors.primary} />
        ))}
        <SvgText x={left} y={height - 6} fontSize="10" fill={Colors.subtext}>
        ...
        <SvgText
          ...
          fill={Colors.subtext}
        >
          {new Date(points[points.length - 1].measuredAt).toLocaleDateString('zh-TW', {
            month: 'numeric',
            day: 'numeric',
          })}
        </SvgText>
