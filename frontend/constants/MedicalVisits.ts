...
export const FOLLOW_UP_STATUS_LABELS:Record<string,string> = { pending:'已建立', snoozed:'已延後', completed:'已完成', skipped:'已略過' };
...
export const buildMedicalVisitShareText = (pet:Pet,visit:MedicalVisit) => {
  ...
  if(visit.medications?.length){lines.push('藥物：');visit.medications.forEach(m=>lines.push(`- ${m.name}${m.instructions?`：${m.instructions}`:''}，每日 ${m.timesPerDay} 次，${MEAL_TIMING_LABELS[m.mealTiming]}`));}
