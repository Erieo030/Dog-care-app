...
import {
  reconcileAccountNotifications, subscribeToNotificationResponses,
...
export default function RootNavigator() {
  ...
  const navigationTheme = dark ? { ...DarkTheme, colors: { ...DarkTheme.colors, primary: '#E9C46A', card: '#241F1B', background: '#191613', text: '#F4EDE4', border: '#3D352F' } } : { ...DefaultTheme, colors: { ...DefaultTheme.colors, primary: '#C98742', card: '#FFFFFF', background: '#FCFAF1', text: '#6A4D3E', border: '#E8E0D4' } };

  const reconcile = useCallback(() => {
    if (!session?.userId || !pets.length) return;
    reconcileAccountNotifications(session.userId, pets).catch(() => undefined);
  }, [session?.userId, pets, settings.localNotificationsEnabled]);
  ...
  useEffect(() => subscribeToNotificationResponses((target) => {
    ...
    navigationRef.navigate('Home', {
      screen: 'ReminderList', params: { focusReminderId: target.reminderId },
    });
  }), [session?.userId, pets, selectPet]);
