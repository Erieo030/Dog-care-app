...
const TYPES: Array<[HealthEventType, string, string]> = [
  ['vomiting', '嘔吐', '🤢'], ['abnormal_stool', '排便異常', '💩'],
  ['low_appetite', '食慾下降', '🍽️'], ['abnormal_drinking', '喝水異常', '💧'],
  ['low_energy', '精神下降', '😴'], ['injury', '受傷', '🩹'],
  ['skin_issue', '皮膚異常', '🔍'], ['eye_ear_issue', '眼睛／耳朵異常', '👀'],
  ['possible_ingestion', '疑似誤食', '⚠️'], ['other', '其他', '📝'],
];
...
export default function AbnormalRecordTypeScreen({ navigation }: Props) {
  return <SafeAreaView style={styles.container}><ScrollView contentContainerStyle={styles.content}>
    ...
    {TYPES.map(([type, label, icon]) => <TouchableOpacity
      key={type} style={styles.item}
      onPress={() => type === 'vomiting'
        ...
        : type === 'abnormal_stool'
          ...
          : ['low_appetite', 'low_energy', 'abnormal_drinking'].includes(type)
            ? navigation.navigate('ObservationHealthEvent', { type: type as ObservationHealthEventType })
            : navigation.navigate('CreateHealthEvent', { type, label })}
    >
      <Text style={styles.icon}>{icon}</Text><Text style={styles.label}>{label}</Text>
...
const styles = StyleSheet.create({
  ...
  item: { minHeight: 64, flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.border, borderRadius: 17, paddingHorizontal: 16, marginBottom: 10 },
  icon: { fontSize: 25, width: 42 }, label: { flex: 1, color: Colors.text, fontWeight: '700', fontSize: 16 },
