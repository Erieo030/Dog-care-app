...
import {
  Alert, Platform, SafeAreaView, ScrollView, StyleSheet, Text, TextInput,
...
import { replaceReminderNotification, scheduleReminderNotification } from '../services/notificationService';
...
export default function CreateReminderScreen({ navigation, route }: Props) {
  ...
  const [scheduledAt, setScheduledAt] = useState(existing ? new Date(existing.scheduledAt) : (() => { const date = new Date(); const [hour, minute] = settings.defaultReminderTime.split(':').map(Number); date.setHours(hour, minute, 0, 0); if (date.getTime() <= Date.now()) date.setDate(date.getDate() + 1); return date; })());
  const [rule, setRule] = useState<RecurrenceRule>(existing?.recurrenceRule ?? 'monthly');
  const [notes, setNotes] = useState(existing?.notes ?? '');
  const [showPicker, setShowPicker] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [clientRequestId] = useState(() => `reminder-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`);
  ...
  const submit = async () => {
    ...
    try {
      const input = {
        type, title: title.trim(), scheduledAt: scheduledAt.toISOString(),
        recurrenceRule: rule, notes: notes.trim(), clientRequestId: existing ? undefined : clientRequestId,
      };
  ...
  return <SafeAreaView style={styles.container}><ScrollView contentContainerStyle={styles.content}>
    <Label text="提醒類型 *" />
    <View style={styles.chips}>{REMINDER_TYPES.map((item) =>
      <Chip key={item[0]} label={item[1]} active={type === item[0]} onPress={() => chooseType(item)} />
    )}</View>
    <Label text="提醒名稱 *" />
    ...
    {showPicker && <DateTimePicker
      value={scheduledAt} mode="datetime" minimumDate={new Date()}
      onChange={(_, date) => { setShowPicker(Platform.OS === 'ios'); if (date) setScheduledAt(date); }}
    />}
    <Label text="是否重複（建議值，可自行調整）" />
    <View style={styles.chips}>{RECURRENCE_RULES.map(([value, label]) =>
      <Chip key={value} label={label} active={rule === value} onPress={() => setRule(value)} />
    )}</View>
    <Text style={styles.notice}>實際頻率請依獸醫建議與產品說明為準。</Text>
    <Label text="備註（選填）" />
    <TextInput style={[styles.input, styles.multiline]} value={notes} onChangeText={setNotes} maxLength={1000} multiline />
    <TouchableOpacity disabled={submitting} style={[styles.submit, submitting && styles.disabled]} onPress={submit}>
      <Text style={styles.submitText}>{submitting ? '儲存中…' : existing ? '儲存修改' : '建立提醒'}</Text>
    </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  chip: { backgroundColor: Colors.surface, borderColor: Colors.border, borderWidth: 1, borderRadius: 16, paddingHorizontal: 13, paddingVertical: 9 },
  chipActive: { backgroundColor: Colors.text, borderColor: Colors.text },
  chipText: { color: Colors.text }, chipTextActive: { color: '#FFF', fontWeight: '700' },
  input: { minHeight: 54, justifyContent: 'center', backgroundColor: Colors.surface, borderColor: Colors.border, borderWidth: 1, borderRadius: 14, paddingHorizontal: 14, color: Colors.text },
  inputText: { color: Colors.text }, multiline: { height: 90, paddingTop: 13, textAlignVertical: 'top' },
