...
import { isObservationType, normalizeObservationDetails, observationDetailRows } from '../constants/ObservationHealthEvents';
import { useAuth } from '../contexts/AuthContext';
import { usePet } from '../contexts/PetContext';
import * as service from '../services/healthEventService';
import { HealthEvent } from '../types';

export default function HealthEventDetailScreen({ route, navigation }: any) {
  ...
  const detailRows = item.type === 'vomiting'
    ...
    : item.type === 'abnormal_stool'
      ...
      : isObservationType(item.type)
        ? observationDetailRows(item.type, normalizeObservationDetails(item.type, item.details ?? {}))
        : Object.entries(item.details ?? {}).map(([label, value]) => [label, String(value)] as [string, string]);
  return <SafeAreaView style={styles.container}><ScrollView contentContainerStyle={styles.content}>
    <View style={styles.actions}>
      <TouchableOpacity disabled={submitting} style={styles.edit}
        onPress={() => navigation.navigate(
          item.type === 'vomiting'
            ? 'VomitingHealthEvent'
            : item.type === 'abnormal_stool'
              ? 'StoolHealthEvent'
              : isObservationType(item.type) ? 'ObservationHealthEvent' : 'HealthEventEdit',
          isObservationType(item.type) ? { eventId: item.id, type: item.type } : { eventId: item.id }
        )}>
    ...
    {!!detailRows.length && <><Text style={styles.heading}>補充資訊</Text>
      {detailRows.map(([label, value]) => <Row key={label} label={label} value={String(value)} />)}
    </>}
    <><Text style={styles.heading}>照片</Text><AttachmentGallery items={item.attachments ?? []} userId={session!.userId} /></>
  </ScrollView></SafeAreaView>;
}

function Row({ label, value }: { label: string; value: string }) {
  return <View style={styles.row}><Text style={styles.label}>{label}</Text>
    <Text style={styles.value}>{value}</Text></View>;
}
...
const styles = StyleSheet.create({
  ...
  edit: { flex: 1, backgroundColor: Colors.primary, padding: 13, borderRadius: 14, alignItems: 'center' },
  editText: { color: '#FFF', fontWeight: '800' },
  delete: { borderWidth: 1, borderColor: '#D96C6C', paddingHorizontal: 20, justifyContent: 'center', borderRadius: 14 },
  deleteText: { color: '#C34D4D', fontWeight: '800' },
  row: { backgroundColor: Colors.surface, borderBottomWidth: 1, borderBottomColor: Colors.border, padding: 15 },
