...
export default function HealthHubScreen({ navigation }: Props) {
  return <SafeAreaView style={styles.container}><View style={styles.content}>
    ...
    <Entry icon="🩺" title="健康異常紀錄" description="查看異常狀況的完整歷史"
      onPress={() => navigation.navigate('HealthEventList')} />
    <Entry icon="⚖️" title="體重紀錄" description="查看趨勢與歷史紀錄"
      onPress={() => navigation.navigate('WeightList')} />
    <Entry icon="🏥" title="就醫紀錄" description="保存看診內容與健康文件"
      onPress={() => navigation.navigate('MedicalVisitList')} />
  </View></SafeAreaView>;
}

function Entry({ icon, title, description, onPress }: {
  icon: string; title: string; description: string; onPress: () => void;
}) {
  return <TouchableOpacity style={styles.card} onPress={onPress}>
    <Text style={styles.icon}>{icon}</Text><View style={styles.cardContent}>
      <Text style={styles.cardTitle}>{title}</Text><Text style={styles.text}>{description}</Text>
    </View>
  </TouchableOpacity>;
}
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.border, borderRadius: 19, padding: 20, marginBottom: 12, flexDirection: 'row', alignItems: 'center' },
  cardContent: { flex: 1 },
