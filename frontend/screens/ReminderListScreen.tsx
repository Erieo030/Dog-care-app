...
import {
  ActivityIndicator, Alert, Platform, RefreshControl, SafeAreaView, ScrollView,
...
import {
  cancelReminderNotifications, reconcileAccountNotifications, replaceReminderNotification,
...
const STATUS_LABEL: Record<Reminder['status'], string> = {
  pending: '待完成', snoozed: '已延後', completed: '已完成', skipped: '已略過',
};
...
export default function ReminderListScreen({ navigation, route }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try { setItems(await service.getReminders(session.userId, selectedPet.id)); }
  ...
  const reconcile = async () => {
    ...
    try { await reconcileAccountNotifications(session.userId, pets); }
    catch { Alert.alert('通知同步失敗', '提醒資料已更新，手機通知將在下次開啟 App 時再次同步。'); }
  };

  const run = async (id: string, task: () => Promise<unknown>) => {
    if (busyId) return;
    setBusyId(id);
    try { await task(); await reconcile(); await load(); }
    catch (requestError) { Alert.alert('操作失敗', (requestError as Error).message); }
    finally { setBusyId(null); }
  };

  const complete = (id: string) => run(id, async () => {
    await service.completeReminder(session!.userId, id);
    await cancelReminderNotifications(id).catch(() => undefined);
  });

  const skip = (item: Reminder) => Alert.alert('略過提醒', `略過「${item.title}」？`, [
    { text: '取消', style: 'cancel' },
    { text: '略過', onPress: () => run(item.id, async () => {
      await service.skipReminder(session!.userId, item.id);
      await cancelReminderNotifications(item.id).catch(() => undefined);
    }) },
  ]);

  const snooze = (item: Reminder) => Alert.alert('延後提醒', item.title, [
    { text: '取消', style: 'cancel' },
  ...
  const runSnooze = (item: Reminder, date: Date) => run(item.id, async () => {
    const updated = await service.snoozeReminder(session!.userId, item.id, date.toISOString());
    await replaceReminderNotification(session!.userId, updated, selectedPet!.name).catch(() => undefined);
  });

  const remove = (item: Reminder) => Alert.alert('刪除提醒', `確定刪除「${item.title}」？`, [
    { text: '取消', style: 'cancel' },
    { text: '刪除', style: 'destructive', onPress: () => run(item.id, async () => {
      await cancelReminderNotifications(item.id).catch(() => undefined);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} refreshControl={
        <RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(); reconcile(); }} />
      }>
        {customItem && <DateTimePicker value={new Date()} mode="datetime" minimumDate={new Date()}
          onChange={(_, date) => {
        ...
        {!items.length ? <Center text="尚未建立提醒" /> : items.map((item) => {
          const active = item.status === 'pending' || item.status === 'snoozed';
          return <View key={item.id} style={[styles.card, route.params?.focusReminderId === item.id && styles.focusCard]}>
            <Text style={styles.title}>{item.title}</Text>
            <Text style={styles.meta}>{new Date(item.scheduledAt).toLocaleString('zh-TW')} · {STATUS_LABEL[item.status]}</Text>
...
function Action({ label, onPress, danger = false, disabled = false }: {
  ...
  return <TouchableOpacity disabled={disabled} style={[styles.action, disabled && styles.disabled]} onPress={onPress}>
    <Text style={[styles.actionText, danger && styles.danger]}>{label}</Text>
  </TouchableOpacity>;
}
function Center({ text, loading, action }: { text: string; loading?: boolean; action?: () => void }) {
  return <View style={styles.center}>{loading && <ActivityIndicator color={Colors.primary} />}
    <Text style={styles.empty}>{text}</Text>
    {action && <TouchableOpacity style={styles.primary} onPress={action}><Text style={styles.primaryText}>重試</Text></TouchableOpacity>}
  </View>;
}
const styles = StyleSheet.create({
  ...
  primary: { backgroundColor: Colors.primary, borderRadius: 15, padding: 15, alignItems: 'center', marginBottom: 16 },
  primaryText: { color: '#FFF', fontWeight: '800' },
  card: { backgroundColor: Colors.surface, borderWidth: 1, borderColor: Colors.border, borderRadius: 18, padding: 17, marginBottom: 12 },
  focusCard: { borderColor: Colors.primary, borderWidth: 2 },
  sourceMissing: { color: '#C55B5B', textAlign: 'center', marginBottom: 12 },
  title: { color: Colors.text, fontSize: 18, fontWeight: '800' }, meta: { color: Colors.subtext, marginTop: 5 },
  notes: { color: Colors.text, marginTop: 9 }, row: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 13 },
  action: { paddingVertical: 8, paddingHorizontal: 11, borderWidth: 1, borderColor: Colors.border, borderRadius: 10 },
  actionText: { color: Colors.text, fontWeight: '700' }, danger: { color: '#E57373' }, disabled: { opacity: 0.5 },
});
