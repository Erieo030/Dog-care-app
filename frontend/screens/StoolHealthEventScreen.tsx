...
import {
  buildStoolSummary, normalizeStoolDetails, shouldShowStoolSafety,
  STOOL_COLOR_OPTIONS, STOOL_CONSISTENCY_OPTIONS, STOOL_SAFETY_MESSAGE,
} from '../constants/Stool';
import { useAuth } from '../contexts/AuthContext';
import { usePet } from '../contexts/PetContext';
import * as service from '../services/healthEventService';
import {
  Attachment, HealthEvent, Severity, StoolColor, StoolConsistency, StoolDetails,
} from '../types';
...
export default function StoolHealthEventScreen({ route, navigation }: any) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      Alert.alert(
        '已儲存',
        safety ? `排便異常紀錄已儲存。\n\n安全提醒：${STOOL_SAFETY_MESSAGE}` : '排便異常紀錄已加入近期動態。',
  ...
  if (loading) return <View style={styles.center}>
    <ActivityIndicator color={Colors.primary} /><Text style={styles.stateText}>正在載入排便異常紀錄…</Text>
  </View>;
  ...
  return <SafeAreaView style={styles.container}><ScrollView contentContainerStyle={styles.content}
    ...
    <OptionGroup options={STOOL_CONSISTENCY_OPTIONS} value={details.stoolConsistency}
      onChange={(stoolConsistency) => setDetails((current) => ({ ...current, stoolConsistency }))} />

    <Text style={styles.label}>顏色 *</Text>
    <OptionGroup options={STOOL_COLOR_OPTIONS} value={details.stoolColor}
      onChange={(stoolColor) => setDetails((current) => ({ ...current, stoolColor }))} />

    <Text style={styles.label}>其他狀況（可複選）</Text>
    <View style={styles.chips}>
      <Toggle text="黏液" active={details.hasMucus}
        onPress={() => setDetails((current) => ({ ...current, hasMucus: !current.hasMucus }))} />
      <Toggle text="疑似血液" active={details.suspectedBlood}
        onPress={() => setDetails((current) => ({ ...current, suspectedBlood: !current.suspectedBlood }))} />
      <Toggle text="異物" active={details.hasForeignObject}
        onPress={() => setDetails((current) => ({ ...current, hasForeignObject: !current.hasForeignObject }))} />
      <Toggle text="疑似蟲體" active={details.suspectedParasite}
        onPress={() => setDetails((current) => ({ ...current, suspectedParasite: !current.suspectedParasite }))} />
    </View>
    ...
    {shouldShowStoolSafety(details, severity) && <View style={styles.safety}>
