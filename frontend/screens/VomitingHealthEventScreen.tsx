...
import {
  ActivityIndicator, Alert, Image, Platform, SafeAreaView, ScrollView,
...
import {
  buildVomitingSummary, DRINKING_OPTIONS, normalizeVomitingDetails, ENERGY_OPTIONS,
  shouldShowVomitingSafety, VOMIT_COLOR_OPTIONS, VOMIT_COUNT_OPTIONS,
...
import {
  EnergyCondition, Attachment, HealthEvent, Severity, VomitCount, VomitingDetails,
} from '../types';
...
export default function VomitingHealthEventScreen({ route, navigation }: any) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setShowAdvanced(Boolean(
        normalized.color || normalized.drinkingCondition || normalized.hasFoam
        || normalized.hasFood || normalized.suspectedBlood
        || normalized.suspectedForeignObject || result.notes
      ));
    } catch (requestError) {
      if (currentRequest !== requestId.current) return;
  ...
  }, [eventId, selectedPet?.id, session?.userId]);
  ...
  const pickImage = async () => {
    if (imageSelecting || submitting || images.length >= 5) return;
    setImageSelecting(true);
    try {
      const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
      if (!permission.granted) {
        Alert.alert('需要權限', '請允許存取相簿以加入照片');
        return;
      }
      ...
      Alert.alert('無法選擇照片', (requestError as Error).message);
    } finally {
  ...
  const submit = async () => {
    ...
    try {
      const input = {
        ...
        details,
        notes: notes.trim(),
        attachmentIds: images.filter(item => item.storageProvider !== 'legacy_local').map(item => item.id),
      };
      ...
      Alert.alert(
        '已儲存',
        safety ? `嘔吐紀錄已儲存。\n\n安全提醒：${VOMITING_SAFETY_MESSAGE}` : '嘔吐紀錄已加入近期動態。',
  ...
  if (loading) return <View style={styles.center}>
    <ActivityIndicator color={Colors.primary} /><Text style={styles.stateText}>正在載入嘔吐紀錄…</Text>
  </View>;
  if (error) return <View style={styles.center}><Text style={styles.error}>{error}</Text>
  ...
  return <SafeAreaView style={styles.container}><ScrollView contentContainerStyle={styles.content}
    ...
    <OptionGroup options={VOMIT_COUNT_OPTIONS} value={details.vomitCount}
      onChange={(vomitCount) => setDetails((current) => ({ ...current, vomitCount }))} />

    <Text style={styles.label}>精神狀況 *</Text>
    <OptionGroup options={ENERGY_OPTIONS} value={details.energyCondition}
    ...
    <TouchableOpacity style={styles.advancedButton} onPress={() => setShowAdvanced((current) => !current)}><Text style={styles.advancedText}>{showAdvanced ? "收合補充資訊" : "補充更多資訊"}</Text></TouchableOpacity>

    {showAdvanced && <View style={styles.advanced}>
      <Text style={styles.label}>顏色（選填）</Text>
      <OptionGroup options={VOMIT_COLOR_OPTIONS} value={details.color}
        onChange={(color) => setDetails((current) => ({ ...current, color }))} />

      <Text style={styles.label}>內容特徵（可複選）</Text>
      <View style={styles.chips}>
        <Toggle text="有泡沫" active={details.hasFoam}
          onPress={() => setDetails((current) => ({ ...current, hasFoam: !current.hasFoam }))} />
        <Toggle text="有未消化食物" active={details.hasFood}
          onPress={() => setDetails((current) => ({ ...current, hasFood: !current.hasFood }))} />
        <Toggle text="疑似有血" active={details.suspectedBlood}
          onPress={() => setDetails((current) => ({ ...current, suspectedBlood: !current.suspectedBlood }))} />
        <Toggle text="疑似有異物" active={details.suspectedForeignObject}
          onPress={() => setDetails((current) => ({ ...current, suspectedForeignObject: !current.suspectedForeignObject }))} />
      </View>

      <Text style={styles.label}>飲水狀況（選填）</Text>
      <OptionGroup options={DRINKING_OPTIONS} value={details.drinkingCondition}
        onChange={(drinkingCondition) => setDetails((current) => ({ ...current, drinkingCondition }))} />

      <Text style={styles.label}>備註（選填，最多 500 字）</Text>
      <TextInput style={[styles.input, styles.notes]} value={notes} onChangeText={setNotes}
        maxLength={500} multiline placeholder="簡短補充觀察到的狀況"
        placeholderTextColor={Colors.subtext} />
    </View>}

    {shouldShowVomitingSafety(details, severity) && <View style={styles.safety}>
      <Text style={styles.safetyTitle}>安全提醒</Text>
      <Text style={styles.safetyText}>{VOMITING_SAFETY_MESSAGE}</Text>
    </View>}

    <TouchableOpacity disabled={submitting || imageSelecting}
      style={[styles.submit, (submitting || imageSelecting) && styles.disabled]} onPress={submit}>
...
function OptionGroup<T extends string>({ options, value, onChange }: {
  ...
  return <View style={styles.chips}>{options.map(([key, label]) =>
    <Toggle key={key} text={label} active={value === key} onPress={() => onChange(key)} />)}</View>;
}
...
const styles = StyleSheet.create({
  ...
  outline: { borderWidth: 1, borderColor: Colors.primary, borderRadius: 14, padding: 14, alignItems: 'center' },
  ...
  submit: { backgroundColor: Colors.primary, borderRadius: 16, padding: 16, alignItems: 'center', marginTop: 24 },
