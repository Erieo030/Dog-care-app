...
const ensurePermission = async (source: AttachmentPickSource) => {
  ...
  if (!result.granted) throw new ApiError(source === 'camera' ? '請允許相機權限後再拍照' : '請允許相簿權限後再選擇照片');
};

const compress = async (asset: ImagePicker.ImagePickerAsset) => {
  const format = asset.mimeType === 'image/png'
  ...
  if (file.size > ATTACHMENT_MAX_BYTES) throw new ApiError('圖片壓縮後仍超過 10 MB，請選擇較小的圖片');
  const mimeType = format === ImageManipulator.SaveFormat.PNG ? 'image/png' : 'image/jpeg';
  if (!(ATTACHMENT_ALLOWED_MIME as readonly string[]).includes(mimeType)) throw new ApiError('只支援 JPG、JPEG、PNG 圖片');
...
export async function pickAndUploadAttachments(input: {
  ...
  const result = input.source === 'camera'
  ...
  for (const asset of selected) {
    ...
    form.append('file', {
      uri: prepared.uri,
      name: asset.fileName || `pawlog-${Date.now()}.${prepared.mimeType === 'image/png' ? 'png' : 'jpg'}`,
      type: prepared.mimeType,
    } as unknown as Blob);
    form.append('width', String(prepared.width));
    form.append('height', String(prepared.height));
    const response = await fetch(`${API_BASE_URL}/api/pets/${input.petId}/attachments?userId=${encodeURIComponent(input.userId)}`, {
      method: 'POST', headers: { Accept: 'application/json' }, body: form,
    });
    if (!response.ok) throw new ApiError((await errorMessage(response)) || `圖片上傳失敗 (${response.status})`, response.status);
...
export const attachmentUri = (item: Attachment, userId: string, retry = 0) => {
  if (item.storageProvider === 'legacy_local' || /^file:|^content:|^https?:/.test(item.contentPath)) return item.contentPath;
...
export async function deleteAttachment(userId: string, item: Attachment) {
  ...
  const response = await fetch(`${API_BASE_URL}/api/attachments/${item.id}?userId=${encodeURIComponent(userId)}`, { method: 'DELETE' });
  if (!response.ok && response.status !== 404) throw new ApiError((await errorMessage(response)) || '刪除照片失敗', response.status);
}
