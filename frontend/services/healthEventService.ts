...
export const getHealthEvent = async (userId: string, eventId: string) =>
  (await apiRequest<{ event: HealthEvent }>(withUser(`/api/health-events/${eventId}`, userId)))
    .event;

export const createHealthEvent = async (userId: string, petId: string, data: HealthEventInput) =>
  (
    await apiRequest<{ event: HealthEvent }>(withUser(`/api/pets/${petId}/health-events`, userId), {
      method: 'POST',
...
export const updateVomitingHealthEvent = async (
  ...
  (
    await apiRequest<{ event: HealthEvent }>(withUser(`/api/vomiting-events/${eventId}`, userId), {
      method: 'PATCH',
      body: JSON.stringify(data),
    })
  ).event;
...
export const createStoolHealthEvent = async (
  ...
  (
    await apiRequest<{ event: HealthEvent }>(withUser(`/api/pets/${petId}/stool-events`, userId), {
      method: 'POST',
      body: JSON.stringify(data),
    })
  ).event;
...
export const updateStoolHealthEvent = async (
  ...
  (
    await apiRequest<{ event: HealthEvent }>(withUser(`/api/stool-events/${eventId}`, userId), {
      method: 'PATCH',
      body: JSON.stringify(data),
    })
  ).event;
...
export const updateObservationHealthEvent = async (
  ...
  (
    await apiRequest<{ event: HealthEvent }>(
      withUser(`/api/observation-events/${eventId}`, userId),
