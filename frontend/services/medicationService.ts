...
export const completeMedication = (u: string, id: string) =>
  apiRequest<{ record: MedicationCourse }>(`/api/medications/${id}/complete?${q(u)}`, {
    method: 'POST',
  });
export const stopMedication = (u: string, id: string) =>
  apiRequest<{ record: MedicationCourse }>(`/api/medications/${id}/stop?${q(u)}`, {
    method: 'POST',
  });
export const deleteMedication = (u: string, id: string) =>
  apiRequest(`/api/medications/${id}?${q(u)}`, { method: 'DELETE' });
