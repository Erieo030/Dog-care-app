...
export const getReminders = async (userId: string, petId: string) =>
  (await apiRequest<{ reminders: Reminder[] }>(withUser(`/api/pets/${petId}/reminders`, userId))).reminders;
export const getTodayReminders = async (userId: string, petId: string) =>
  (await apiRequest<{ reminders: Reminder[] }>(withUser(`/api/pets/${petId}/reminders/today`, userId))).reminders;
...
export const completeReminder = (userId: string, id: string) =>
  apiRequest<ReminderActionResult>(withUser(`/api/reminders/${id}/complete`, userId), { method: 'POST' });
export const skipReminder = (userId: string, id: string) =>
  apiRequest<ReminderActionResult>(withUser(`/api/reminders/${id}/skip`, userId), { method: 'POST' });
