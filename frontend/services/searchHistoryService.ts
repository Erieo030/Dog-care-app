...
export async function getSearchHistory(userId: string): Promise<string[]> {
  try {
    ...
    const value = JSON.parse(await target.text());
...
export async function addSearchHistory(userId: string, query: string) {
  ...
  if (!value) return getSearchHistory(userId);
  const current = await getSearchHistory(userId);
  const next = [value, ...current.filter((x) => x !== value)].slice(0, 10);
  file(userId).write(JSON.stringify(next));
  return next;
}
